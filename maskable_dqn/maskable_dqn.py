...
from maskable_dqn.policy import MaskableDQNPolicy, MaskableQNetwork
...
class MaskableDQN(DQN):
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            action_masks = get_action_masks(env).astype(np.int8)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            # Warmup phase
            unscaled_action = np.array([self.action_space.sample(action_masks[i]) for i in range(n_envs)])
        else:
    ...
    def predict(
        ...
        if not deterministic and np.random.rand() < self.exploration_rate:
            if self.policy.is_vectorized_observation(observation):
                ...
                action = np.array([self.action_space.sample(action_masks[i]) for i in range(n_batch)])
            else:
                action = np.array(self.action_space.sample(action_masks))
        else:
